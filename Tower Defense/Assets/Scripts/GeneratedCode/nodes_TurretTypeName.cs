...
{
	public class nodes_TurretTypeName : Sensor
	{
		private int counter;
		...
		public override void Initialize(SensorConfiguration sensorConfiguration)
		{
			...
			counter = 0;
			mappingTemplate = "nodes_TurretTypeName(sensors,objectIndex("+index+"),{1},{0})." + Environment.NewLine;
		}
		...
		public override void Update()
		{
			...
			if(!invariant || first)
			{
				...
				List<NodeSensorData> Nodes_2 = SensorsDataListsManager_1.Nodes;
				if(Nodes_2 == null)
				{
					values.Clear();
					return;
				}
				else if(Nodes_2.Count > values.Count)
				{
					for(int i = values.Count; i < Nodes_2.Count; i++)
					{
				...
				{
					for(int i = Nodes_2.Count; i < values.Count; i++)
					{
						values.RemoveAt(values.Count - 1);
					}
				}
				for(int i_2 = 0; i_2 < Nodes_2.Count; i_2++)
				{
					if(Nodes_2[i_2] == null)
					{
						values[i_2].Clear();
					...
					string TurretTypeName_3 = Nodes_2[i_2].TurretTypeName;
					if(TurretTypeName_3 == null)
					{
						values[i_2].Clear();
						continue;
					}
					...
					values[i_2].Add(TurretTypeName_3);
				}
			}
		}
